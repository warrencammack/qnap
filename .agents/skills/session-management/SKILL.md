...
### 1. AGENTS.md as Entry Point
AGENTS.md must reference session-management.md in the Skills section. Codex reads AGENTS.md first, which directs it to follow session rules.
...
After completing any task, Codex should ask:
```
...
When starting a session, Codex must:
...
Every ~20 tool calls, Codex should check:
...
- "What's the current state?" → Codex reads and reports
- "End session" → Triggers archive + handoff
- "Resume from last session" → Codex reads state files first
